...
**Deterministic trace id (feedback → score loop)**: `factory.py::build_langfuse_trace_id(run_id)` derives a stable Langfuse trace id from a run id via `Langfuse.create_trace_id(seed=run_id)` (returns `None` when Langfuse is disabled or run_id is empty). `runtime/runs/worker.py::run_agent` computes it and stashes it in `config["configurable"]["langfuse_trace_id"]`; `make_lead_agent` reads it and passes it to `build_tracing_callbacks(langfuse_trace_id=...)`, which constructs the Langfuse `CallbackHandler(trace_context={"trace_id": ...})` so the whole run lands on that exact trace (custom trace id can only be set at handler construction in langfuse v4 — not via metadata). This lets `app/gateway/routers/feedback.py` recompute the same trace id from `run_id` and call `get_langfuse_client().create_score(name="user_feedback", value=±1, trace_id=..., data_type="NUMERIC")` on feedback create/upsert — best-effort, never blocks the endpoint. `factory.py::get_langfuse_client()` builds the client from the same tracing config (host from `LANGFUSE_BASE_URL`) for non-callback callers.

**Eval harness**: `benchmark/` (project root) holds server-run scripts that build on this wiring, organized by eval layer since 2026-07-11 — shared infra in `benchmark/_shared/` (`smoke_test.py` verify traces land with the right tags, `upload_datasets.py` push golden sets to Langfuse Datasets, `probe_gateway.py` single-query probe), per-layer runners live next to their data (e.g. `benchmark/L1_routing/run_routing_experiment.py` scoring routing/clarify from agent tool calls). See `benchmark/README.md` (directory map) and `benchmark/_shared/README.md`.
...
- `lead_agent.system_prompt_path` - Optional path to a system-prompt template file for the default lead agent (relative to project root or absolute). `null` (default) uses the built-in `SYSTEM_PROMPT_TEMPLATE`. Resolved per request in `apply_prompt_template()` via `_resolve_system_prompt_template()`, so editing it swaps the prompt variant on the next message (hot-reloadable; no restart). A missing/unreadable file logs a warning and falls back to the built-in default. The override file may only use `{placeholders}` that are a subset of `apply_prompt_template`'s format kwargs. **File format**: `.yaml`/`.yml` files are parsed and the top-level `system_prompt:` field is used as the template (single block-scalar, editable in place); any other extension (`.md`/`.txt`) is read whole-file as the template. The variant tag is `Path(path).stem`, so the extension does not affect it. CE prompt variants live in `benchmark/prompts/lead_agent_v*.yaml`.
